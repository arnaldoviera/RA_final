...
grafico1<-ggpairs(data1,aes(alpha = 14.5), 
                  ...
                  title="Grafico 1: Correlación entre las variables") 
...
Verificación de los supuestos del modelo 4:

El análisis visual de la relación entre las variable respuesta y cada una de las explicativas no es perfectamente lineal (está en el código interno). Por otro lado, el gráfico de los residuos en función del ajuste aparenta ser una nube de puntos sin identificar algún comportamiento de sospecha. Por otro lado, se realizó el test de Durbin-Watson para evaluar la independecia de los residuos y se confimó que tales residuos son independientes.
...
Siguiendo con el mismo procedimiento de verificación de lo supuestos, en este modelo 4 se observa que la observación 1602 (correspondiente al condado de Kings, es decir, Brookling) registra el máximo valor de crimen y densidad. Por tanto, se decide excluir esta observacón del análisis y de esta manera queda conformado el último modelo (mod_4_A).
...
library(lmtest)
bptest(mod_4_A)
#BP = 151.13, df = 32, p-value < 2.2e-16
```
...
Podemos observar que en el summary del modelo las variables **ancianos** y **crimen** no son significativas como así tambien algunos estados. 


Se realizó un test para evaluar si se decide mantener las variables ancianos y crimen en el modelo. Por tal motivo, se construtó el Test de Suma de Cuadrados Extra:


```{r, include=FALSE, eval=TRUE, echo=TRUE}

mod_4_B <- lm(formula = pje ~ ahorros + ingpc + pobreza + veteranos + mujeres + densidad + 
                data_dummy_AR + data_dummy_AZ + data_dummy_CO + data_dummy_CT + data_dummy_DC + data_dummy_DE + data_dummy_FL + data_dummy_GA + data_dummy_IA +
                data_dummy_ID + data_dummy_IL + data_dummy_IN + data_dummy_KS + data_dummy_LA + data_dummy_MA +
                data_dummy_MN + data_dummy_MS + data_dummy_MT + data_dummy_NC + data_dummy_ND + data_dummy_NE + data_dummy_NH + data_dummy_NM +
                data_dummy_NV + data_dummy_OH + data_dummy_OK + data_dummy_OR + data_dummy_RI + data_dummy_SD + data_dummy_TN + 
                data_dummy_TX + data_dummy_UT + data_dummy_VA + data_dummy_VT + data_dummy_WI + data_dummy_WV + data_dummy_WY, data=data_dummy_1)

summary(mod_4_B)
anova(mod_4_B)
anova(mod_4_B, mod_4_A)
...
El test F nos indica no rechazar la Ho), es decir, la reducción en la suma de cuadrados del error tras incorporar las dos variables no es significativa.

Por tanto, el modelo elegido es el **mod_4_B**. La estructura es la siguiente:

CAMBIE DE MODELO
pje = -7.490 - 0.000024 ahorros + 0.00016 ingpc + 0.83329 pobreza + 0.4121 veteranos + 0.558 mujeres +0.00218 densidad + 0.013 ancianos + 0.00154 crimen + 10.43 data_dummy_AR -5.54 data_dummy_AA - 3.488 data_dummy_CO + 5.8 data_dummy_CT
+17.71 data_dummy_DC + 7.156 data_dummy_DE -5.734 data_dummy_FL + 0.846 data_dummy_GA + 4.208 data_dummy_IA -10.52 data_dummy_ID + 6.194 data_dummy_IL - 1.902 data_dummy_IN - 8.118 data_dummy_KS - 1.936 data_dummy_LA + 9.876 data_dummy_MA + 3.013 data_dummy_MN -6.07 data_dummy_MS -5.284 data_dummy_MT +
3.839 data_dummy_NC -4.44 data_dummy_ND - 10.74 data_dummy_NE + 5.048 data_dummy_NH - 2.74 data_dummy_NM -5.048 data_dummy_NV - 2.983data_dummy_OH -5.992 1data_dummy_OK -2.786 data_dummy_OR + 7.255 data_dummy_RI - 2.322 data_dummy_SD + 6.828 data_dummy_TN -5.933 data_dummy_TX - 14.86 data_dummy_UT + 0.57 data_dummy_VA + 8.238 data_dummy_VT + 3.464 data_dummy_WI + 1.961 data_dummy_WV -7.701 data_dummy_WY

Este modelo presenta un R2 ajustado de 0.55 (es decir, el 55% de la de variación de la variable pje es explicado colectivamente por todas las variables independientes) y significatividad en la mayoría de las variables.

Interpretación de los coeficientes del modelo:

Por ejemplo la variable **pobreza**  la podemos interpretar como "ante un incremento en el porcentaje de la población bajo la línea de pobreza en ese condado, el porcentaje de votos se incrementa en 0.85 manteniendo el resto de las variables constantes. Por otro lado, para la variable **mujeres** el significado del coeficiente seria: "ante un incremento en el porcentaje de mujeres en ese condado, el porcentaje de votos se incrementa en 0.55 manteniendo el resto de las variables constantes.
El resto de las variables (ingpc, veteranos, densidad) demuestran una influencia mucho menor.
Por último, analizando los estados y considerando aquellos estados con mayores coeficientes asociados, en términos absolutos, se observa que estados como  *Arkansas*, *Distrio de Columbia*, *Idaho*, *Nebraska* y *Utah* son las más importantes. Esto significa que, por ejemplo, si un votante reside en Utah, tiene una probabilidad menor en 14.86 en pje de votar a Clinton dada específicamente por su lugar de residencia, si todas las variables restantes se mantienen constantes. El estado influye negativa y fuertemente en la probabilidad de voto. En el caso de Arkansas, los residentes de ese estado tienen una probabilidad de voto de un 10.43 pje mayor por encontrarse en dicho estado, si todas las variables restantes se mantienen constantes. El intercepto no resulta interpretable a los fines del problema bajo estudio, ya que no tiene sentido decir que un modelo que sólo comprende la constante determina que el individuo tendrá una probabilidad de voto negativa.
...
El siguiente cuadro especifica las variables que son significativas en el modelo logit:
...
Aqui podemos ver que algunas variables que eran significativas en la regresión lineal múltiple, ahora en la regresión logística no lo son. Por ejemplo...
Antes teniamos ??  significativos y ahora tenemos ??

Al tratarse de una regresión logística, la interpretación de los coeficientes adquieren ciertas particularidades. Las variables con signo positivo tendrán impacto en dicha dirección sobre la variable predicha, mientras que lo contrario sucederá con las negativas. Aquellas de mayor magnitud impactarán proporcionalmente más que las de menor magnitud relativa, pero dicho impacto no será lineal (a diferencia del punto anterior).
...
Comparando el ajuste global de este modelo respecto del modelo de regresión múltiple seleccionado en la parte I, podemos decir que el modelo de regresión logístico contiene a menos variables significativas. 

Por último, el análisis de la bondad del ajuste nos muestra que este modelo tiene una Null deviance de 2251.8 con 2701 grados de libertad y Residual deviance de 1458.6 con 2656 grados de libertad.
...
### 3. 1. Comparación de las predicciones con las observaciones (ANALIZAR BIEN ESTO)
